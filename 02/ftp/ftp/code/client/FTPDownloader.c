...
int main1() {
    ...
    config_t config;
    config.user = "AgUser";
    config.password = "@E#Wscdx";
    config.host = "ftp.bcnyyun.com";
    config.url_path = "/7bf0753bbd494bed8dadfdfb7a7c3f43-2-1.0.5";
    config.localPath = "/Users/delims/Desktop/1234";
...
//    sprintf(filepath, "%s/Downloads/%s", getenv("HOME"), basename(config->url_path));
    ...
    FILE* file = fopen(config->localPath, "w+");
    if (file == NULL) return -1;
    
    printf(" %s ... ", config->localPath);
